...
// point = [lat, lon]
// path = [ (distPlus0, thetaPlus0), ..., (distPlus11, thetaPlus11)]
...
Type::Path FlatEarthPathCalculator::eval() const {
    ...
    for (size_t i = 1; i < _path.size(); ++i) {
        Type::Point p_next = { 
            _data.p0[0] + (_data.d * _path[i].first) * cos(math::degreeToRadian(_data.th + _path[i].second))*NM_TO_LL, 
            _data.p0[1] + (_data.d * _path[i].first) * sin(math::degreeToRadian(_data.th + _path[i].second))*NM_TO_LL / cos(math::degreeToRadian(_data.p0[0]))
        }; 
...
Type::Path SphericalEarthPathCalculator::eval() const {
    ...
    for (size_t i = 1; i < _path.size(); ++i) {
        double hav_rad = _data.d * _path[i].first / physics::EARTH_RADIUS_NM;
        double theta_rad = math::degreeToRadian((_data.th + _path[i].second));
        double lat_rad = math::degreeToRadian(_data.p0[0]) ;

        Type::Point nextPonit;

        nextPonit[0] = math::radianToDegree((sin(lat_rad) * cos(hav_rad) + cos(lat_rad) * sin(hav_rad) * cos(theta_rad))) ;
        nextPonit[1] = _data.p0[0] + math::radianToDegree((sin(theta_rad) * sin(hav_rad) * cos(lat_rad),
                                            cos(hav_rad) - sin(lat_rad) * sin(math::degreeToRadian(nextPonit[0]))));
...
Type::Path WGS84PathCalculator::eval() const {
    ...
    for (const auto& step : _path) {
        ...
        const double U1 = atan((1 - f) * tan(math::degreeToRadian(p_current[0])));
        const double U2 = atan((1 - f) * tan(math::degreeToRadian(p_current[0]))); // Note: should be the new latitude after calculation
        const double L = math::degreeToRadian(p_current[1]) - bearing; // Difference in longitude
        ...
        double lon2 = math::degreeToRadian(p_current[1]) + (1 / cosU1) * (distance / (a * (1 - f))) * sinLambda;

        // Update current point
        p_current[0] = math::radianToDegree(lat2);
        p_current[1] = math::radianToDegree(lon2);
